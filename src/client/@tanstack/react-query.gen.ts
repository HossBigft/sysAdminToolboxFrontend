...
export const getPublicNsPropagationQueryKey = (options: OptionsLegacyParser<GetPublicNsPropagationData>) => [
    createQueryKey('getPublicNsPropagation', options)
];

export const getPublicNsPropagationOptions = (options: OptionsLegacyParser<GetPublicNsPropagationData>) => {
    return queryOptions({
        queryFn: async ({ queryKey, signal }) => {
            const { data } = await getPublicNsPropagation({
                ...options,
                ...queryKey[0],
                signal,
                throwOnError: true
        ...
        queryKey: getPublicNsPropagationQueryKey(options)
    });
};
